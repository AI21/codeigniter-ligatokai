...
//print nl2br(print_r($aryMvpPublicList,1));
	...
	//					$aryMvpCandidatePlayers[$i]['score']['TO'] 	= $intScore3P + $intScore2P + $intScoreFT;
						...
						if ( $aryMvpCandidatePlayers[$i]['mvp_flg'] == '1' ) {
							...
							// 背番号
							$aryMvpPlayerData['comment'] = $aryMvpCandidatePlayers[$i]['comment'];
							// 個人得点とスタッツ
	...
	//print nl2br(print_r($arySectionGameData,1));
			...
			if ( $intSectionGameDataCnt > 0 ) {
				for ( $i = 0; $i < $intSectionGameDataCnt; $i++ ) {
					...
					// 候補選手がない試合はMVP選手の試合
					if ( isset($arySectionGameData[$i]['mvpCandidatePlayer']) == FALSE ) {
	//					$arySectionGameData[$i]['mvpCandidatePlayer'] = NULL;
					...
					// 試合日＆試合会場
					if ($strGameDay == '') {
	//					$strGameDay = $arySectionGameData[$i]['game_day'] . ' ' . $arySectionGameData[$i]['h_name'];
						$strGameDay = $arySectionGameData[$i]['game_day'];
						$strGameHall = $arySectionGameData[$i]['h_name'];
					}
	//				if ( $strGameDay != $arySectionGameData[$i]['game_day'] . ' ' . $arySectionGameData[$i]['h_name'] ) {
	//					$strGameDay .= '<br>'.$arySectionGameData[$i]['game_day'] . ' ' . $arySectionGameData[$i]['h_name'];
	//				}

					// MVP選手のゲームIDを取得
	//				if ( $aryMvpPlayerData['team_id'] > 0 ) {
	//					if ( $aryMvpPlayerData['team_id'] == $arySectionGameData[$i]['home_team'] || $aryMvpPlayerData['team_id'] == $arySectionGameData[$i]['away_team'] ) {
	//						$intMvpPlayerGameId = $arySectionGameData[$i]['game_id'];
	//					}
	//				}
				}
	//			$arySectionGameData = array_values($arySectionGameData);
			}
