<?php $strBreakName = ''; ?>
<?php //print nl2br(print_r($arySponsor,1)); ?>
		<header>
			<h1>リーガ東海 1部 第<?= $strSectionNo; ?>節</h1>
			<h2><?= $arySponsor['sponsor_view']; ?> プレゼンツ</h2>
			<p>
				<time></time><?= $strGameDay; ?> <?= $strGameHall; ?>
			</p>
...
?>
						<li><a href="<?= LEAGUE_DIR_MVP ?>/<?= $strPublicList['r_id']; ?>/1/<?= $strPublicList['section_no']; ?>/">第<?= $strPublicList['section_no']; ?>節</a></li>
<?php endforeach;?>
					</ol>
				</nav>
			</div>
								...
								<div class="resultGame">
									<table>
										<tr class="home">
											...
											<td rowspan="2"><button onclick="javascript:openWindow(<?= $resultGame['game_id']; ?>);">ゲーム詳細</button></td>
										</tr>
										<tr class="away">
...
<?php /*
								<span class="teams txr"><?= $game['home_team_name']; ?></span>
								<span class="result txc"><span class="<?= $game['resultHome']; ?>"><?= $game['score_home']; ?></span> - <span class="<?= $game['resultAway']; ?>"><?= $game['score_away']; ?></span></span>
								<span class="teams txl"><?= $game['away_team_name']; ?></span>
*/ ?>
							</li>
<?php endforeach;?>
...
<?php if ( isset($arySponsor['goods_image']) == TRUE) : // 提供商品 ?>
									<img src="<?= $arySponsor['goods_image']; ?>" alt="<?= $arySponsor['sponsor_view']; ?>様提供「<?= $arySponsor['goods_name']; ?>」" width="140">
<?php endif;?>
									<?= $aryMvpPlayerData['player_name_first']; ?>選手には、冠協賛の<?= $arySponsor['sponsor_view']; ?>様より「<?= $arySponsor['goods_name']; ?>」が進呈されます！
								</p>
							</li>
						</ol>
					</section>
